...
@app.route('/user')
def user():
    conn = sqlite3.connect('scraped_data.db', check_same_thread=False)
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM scraped_data')
    data = cursor.fetchall()
    conn.close()

    # Process data to be used in the template
    scraped_data = []
    for row in data:
        scraped_data.append({
            'url': row[1],
            'emails': row[2].split(', '),
            'mobiles': row[3].split(', '),
            'social_links': row[4].split(', '),
            'linked_emails': row[5].split(', '),
            'linked_mobiles': row[6].split(', ')
        })

    # Ensure you return the rendered template
    return render_template('user.html', scraped_data=scraped_data)
